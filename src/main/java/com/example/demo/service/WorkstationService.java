...
import com.example.demo.repository.WorkstationSlotRepository;
...
public class WorkstationService {
    ...
    @Transactional(readOnly = true)
    public Workstation findByIdOrThrow(Long id) {
        return workstationRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Unknown workstation"));
    }
    ...
    public List<WorkstationSlot> getSlotsOrderedElq(Workstation w) {
        ...
        list.sort(
                (a, b) -> {
                    int o = Integer.compare(a.getRole().ordinal(), b.getRole().ordinal());
                    return o != 0 ? o : Long.compare(a.getId() == null ? 0 : a.getId(), b.getId() == null ? 0 : b.getId());
                });
        return list;
    }
